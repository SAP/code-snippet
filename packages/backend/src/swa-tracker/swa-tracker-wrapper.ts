...
export class SWA {
	...
	public static createSWATracker(logger?: IChildLogger) {
		try {
			...
			if (logger) {
				logger.info(`SAP Web Analytics tracker was created for ${SWA.CODE_SNIPPET}`);
			}
		} catch (error) {
			logger.error(error);
		}
	...
	public static updateSnippetStarted(snippetName: string, logger?: IChildLogger) {
		try {
			if (SWA.isInitialized()) {
				...
				if (logger) {
					logger.trace("SAP Web Analytics tracker was called and start time was initialized", {
						eventType, snippetName, startTime: SWA.startTime, customEvents});
				}
			}
		} catch (error) {
			logger.error(error);
	...
	public static updateSnippetEnded(snippetName: string, isSucceeded: boolean, logger?: IChildLogger, errorMessage?: string) {
		try {
			if (SWA.isInitialized()) {
				const eventType = isSucceeded ? 
					SWA.EVENT_TYPES.SNIPPET_CREATION_SUCCESSFULLY : SWA.EVENT_TYPES.SNIPPET_CREATION_FAILED;
				...
				if (logger) {
					logger.trace("SAP Web Analytics tracker was called", 
						{eventType, snippetName, creationTimeSec, creationTimeMilliSec, endTime, startTime: SWA.startTime, customEvents, errorMessage});
				}
			}
		} catch (error) {
			logger.error(error);
